...
export class ExtendedBSC extends NativeCurrency {
  protected constructor(chainId: number) {
  ...
  private static _cachedXDC: { [chainId: number]: ExtendedBSC } = {};
  static onChain(chainId: number): ExtendedBSC {
    return this._cachedXDC[chainId] ?? (this._cachedXDC[chainId] = new ExtendedBSC(chainId));
  }
