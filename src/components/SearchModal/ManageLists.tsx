...
import { acceptListUpdate, disableList, enableList, removeList } from '../../store/lists/actions';
import { useActiveListUrls, useAllLists, useIsListActive } from '../../store/lists/hooks';
...
import { ButtonEmpty, ButtonPrimary } from '../Common/Button';
import ListLogo from '../Common/ListLogo';
import { Span } from '../Common/Span';
import ListToggle from '../Common/Toggle/ListToggle';
      ...
      Object.values(lists).reduce((acc, { current: list }) => {
        ...
        return {
          ...
          [list.name]: list.tokens.reduce(
            (count: number, token) => (token.chainId === selectedChainId ? count + 1 : count),
            0,
          ),
        };
      }, {}),
    [selectedChainId, lists],
  );
      ...
      <ListContainer>
        <AutoColumn gap='md'>
          {sortedLists.map((listUrl) => (
            <ListRow key={listUrl} listUrl={listUrl} selectedChainId={selectedChainId} />
          ))}
        </AutoColumn>
      </ListContainer>
    </Wrapper>
